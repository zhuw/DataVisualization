...
var dashboard5 = (function () {
    ...
    function createBoard()
    {
        ...
        var margin = {top: 50, right:15, bottom: 10, left: 15},
            width = 1150 - margin.left - margin.right,
            height = 400 - margin.top - margin.bottom;


        var csvData, startYear, endYear, yearDividers, playerCircles;
        var cities = ["Phoenix","Mesa","Chandler","Scottsdale","Tempe"];
        var yPositions = {};
        var xPositions = {};
        var cityCount={};
        var radius = 4;
...
//Custom interpolation so faded colors are assigned to round > 10
        var categoryRange = d3.scale.category10().range();
...
//Use 2 groups for 2 separate graph views
        ...
        individualLegends.selectAll("circle")
            .data(_.range(1, 5.5,0.5))
            .enter().append("circle")
            .attr("r", radius)
            .attr("cx", function(d, i) { return 100 + (i * 40); })
            .attr("cy", -14)
            .style("fill", function(d) { return color(2*d); });
        ...
        function filterByPosition(price) {
            if ( price == 'All' ) {
                playerCircles.transition().attr('r', radius).duration(500);
            } else {
                playerCircles.each(function(d) {
                    if ( d.price_range != price ) {
                        d3.select(this).transition().attr('r', 0).duration(500);
                    } else {
                        d3.select(this).transition().attr('r', radius).duration(500);
                    }
        ...
        function loadCSV() {
            var file = "data/price_star.csv";
            cleanup();
            _.each(cities, function(city) {
                cityCount[city] = {'NonChinese':[0],'Chinese':[0]};
            });
            d3.csv(file, function(error, data) {
                csvData = data.filter(function (r) {
                    if(r.Chinese=="TRUE")
                        cityCount[r.city]['Chinese']++;
                    else
                        cityCount[r.city]['NonChinese']++;
                    return r
                });
                attachXYpositions();
                draw();
...
//Memoize the yPosition into d itself so it can be re-used in transitions without recomputing every time
        function attachXYpositions() {
            _.each(cities, function(city) {
                yPositions[city] = {'NonChinese':[1],'Chinese':[1]};
                xPositions[city] = {'NonChinese':[1],'Chinese':[1]};
            });
            csvData.forEach(function(restaurant) {
                restaurant.yPosition = yearY(restaurant);
                restaurant.xPosition = yearX(restaurant);
            })
        }
...
//1. assume data is already bound to playerCircles
        ...
        function draw() {
          ...
          //  setupYears(startYear, endYear);
            ...
            playerCircles = individual.selectAll(".playerCircle")
                ...
                .attr("cy", height/2)
                .style("fill", function(d) { return color(2*d.stars); })
...
//For given year value, return the x position
        function yearX(d) {
            var yearWidth = (width - leftBuffer)/5;
            var counter,last,lastx,available,resume;
            if(d.city=="Phoenix")
            {
                counter=0;
            }else if(d.city=="Mesa")
            {
                counter=1;
            }else if(d.city=="Chandler")
            {
                counter=2;
            }else if(d.city=="Scottsdale")
            {
                counter=3;
            }else if(d.city=="Tempe")
            {
                counter=4;
            }
            if(d.Chinese=="TRUE")
            {
                last = _.last(yPositions[d.city]['Chinese'])-1;
                lastx=_.last(xPositions[d.city]['Chinese']);
                if(last==9)
                {
                    xPositions[d.city]['Chinese'].push(lastx+1);
                }
                available=leftBuffer + yearWidth * counter+lastx*20;
            }else
            {
                last = _.last(yPositions[d.city]['NonChinese'])-1;
                lastx=_.last(xPositions[d.city]['NonChinese']);
                if(last==9)
                {
                    xPositions[d.city]['NonChinese'].push(lastx+1);
                }
                available=leftBuffer + yearWidth * counter+lastx*20;
            }
...
//For given year value, retuen the position
        ...
        function yearY(d) {
            ...
            if(d.Chinese=="TRUE")
            {
                last = _.last(yPositions[d.city]['Chinese']);
                if(last==10)
                {
                    last=1;
                }
                priceheight= height/2+last*2*radius;
                last++;
                yPositions[d.city]['Chinese'].push(last);
            }else
            {
                last = _.last(yPositions[d.city]['NonChinese']);
                if(last==10)
                {
                    last=1;
                }
                priceheight= height/2-last*2*radius;
                last++;
                yPositions[d.city]['NonChinese'].push(last);
            }
            return priceheight;
        }
        ...
        function setupCities() {
            //Clear existing
            var yearWidth =(width - leftBuffer) / cities.length;
    ...
    function render() {
        ...
        '<script src="//code.jquery.com/ui/1.10.4/jquery-ui.js"></script>'+
            '<h1 align="center">Price and Star</h1>' +
            '<div class="row">'+
            ...
            '<button class="btn btn-small active">Star</button> ' +
            '<button class="btn btn-small">Price Range</button> ' +
            '</div> ' +
            '</div> ' +
            ...
            '<li><a href="#3">2</a></li> ' +
            '<li><a href="#4">2.5</a></li> ' +
            '<li><a href="#5">3</a></li> ' +
            '<li><a href="#6">3.5</a></li> ' +
            '<li><a href="#7">4</a></li> ' +
            '<li><a href="#8">4.5</a></li> ' +
            '<li><a href="#8">5</a></li> ' +
            '</ul> ' +
            ...
            '<li><a href="#">4</a></li> ' +
            '</ul> ' +
            '<ul class="nav nav-pills" id="openhour-links" style="display: none;"> ' +
            '<li class="active"><a href="#all">All</a></li> ' +
            '<li><a href="#">1</a></li> ' +
            '<li><a href="#">2</a></li> ' +
            '<li><a href="#">3</a></li> ' +
